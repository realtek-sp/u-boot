 ...
 *----------------------------------------
 */
int dma_copy(u32 src_addr, u32 dst_addr, u32 trans_length)
{
	u32 rd_data;
	...
	REG32(DMA_CLK_CFG_REG) = REG32(DMA_CLK_CFG_REG) | 0x01;
	REG32(DMA_CLK_CFG_REG);
	/*select SRC_TR_WIDTH->64 bits
	 *select BLOCK_TS->2048
	 *so Block Transfer Size = 8K Bytes
	 */
	while (trans_length) {
		if (trans_length > 16384)
			cur_transfer_size = 16384;
		else
		...
		ltmp = cur_transfer_size / 8;
		/*
		 *bit 3:1 DST_TR_WIDTH 010 32bit
		 *bit 6:4 SRC_TR_WIDTH 010 32bit
		 *bit 8:7 DINC 00	increment
		 *bit 10:9 SINC 00 increment
		 *bit 13:11 DEST_MSIZE 011 16
		 *bit 16:14 SRC_MSIZE	011	16
		 *bit 43:32 BLOCK_TS
		 */
		REG64(CTL0) = (ltmp << 32) | 0xd837;

		/*3.e Program CFG
		 *bit33 FIFO_MODE Select
		 */
		/*3.e Program CFG*/
		REG64(CFG0) = 0x00;
	...
	REG32(DMA_CLK_CFG_REG) = (REG32(DMA_CLK_CFG_REG) & 0xffffffe) | 0x02;
