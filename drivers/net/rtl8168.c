...
#undef _ETH_OOB_TEST_
#undef DEBUG_RTL8168_INIT
/*#define _ETH_OOB_TEST_*/
/*#define _ETH_LOOPBACK_TEST_*/
#ifdef CONFIG_BOARD_rle0745
#define _ETH_PATCH_
#endif

#define rtd_inl(addr)		(*(volatile u32 *)(addr))
...
/* max supported gigabit ethernet frame size
 *-- must be at least (dev->mtu+14+4).
 */
...
/* write/read MMIO register. Notice: {read,write}[wl]
 *do the necessary swapping
 */
#define RTL_W8(reg, val8)	writeb((val8), (u8 *)(ioaddr + (reg)))
#define RTL_W16(reg, val16)	writew((val16), (u16 *)(ioaddr + (reg)))
#define RTL_W32(reg, val32)	writel((val32), (u32 *)(ioaddr + (reg)))
#define RTL_R8(reg)		readb((u8 *)(ioaddr + (reg)))
#define RTL_R16(reg)		readw((u16 *)(ioaddr + (reg)))
#define RTL_R32(reg)		((unsigned long)readl((u32 *)(ioaddr + (reg))))
...
#define bus_to_phys(a)	pci_mem_to_phys((pci_dev_t)dev->priv, (unsigned long)a)
#define phys_to_bus(a)	pci_phys_to_mem((pci_dev_t)dev->priv, (unsigned long)a)
...
#if defined(CONFIG_ETH_EFUSE_INIT)
extern unsigned char efuse_read_byte(unsigned int addr);
#endif

/*POLY= 2^32 + 2^7 + 2^5 + 2^3 + 2^2 + 2^1 + 1*/
const unsigned int POLY16 = 175;
...
static int rtl8168_recv(struct udevice *dev, int flags, uchar **packetp)
{
	...
	*/
	...
	invalidate_dcache_range(rounddown((unsigned long)addr, ARCH_DMA_MINALIGN),
					roundup((addr + size), ARCH_DMA_MINALIGN));
	...
	if ((le32_to_cpu(tp->RxDescArray[cur_rx].opts1) & DescOwn) == 0) {
		...
		if (!(le32_to_cpu(tp->RxDescArray[cur_rx].opts1) & RxRES)) {
			...
			invalidate_dcache_range(rounddown((unsigned long)tp->RxBufferRing[cur_rx], ARCH_DMA_MINALIGN),
					roundup((unsigned long)tp->RxBufferRing[cur_rx]+length, ARCH_DMA_MINALIGN));
			dmb();
			*packetp = tp->RxBufferRing[cur_rx];
		...
		*/
		...
		if ((cur_rx & size) == size) {
			...
			flush_cache(rounddown((unsigned long)&tp->RxDescArray[cur_rx-size], ARCH_DMA_MINALIGN),
					    ARCH_ALIGN);
		}
...
static void rtl_inval_tx_desc(struct TxDesc *desc)
{
	...
	invalidate_dcache_range(rounddown((unsigned long)start, ARCH_DMA_MINALIGN),
...
static int rtl8168_send(struct udevice *dev, void *packet, int length)
{
...
#endif
	flush_cache(rounddown((unsigned long)packet, ARCH_DMA_MINALIGN),
	...
	dsb();
	flush_cache(rounddown((unsigned long)&tp->TxDescArray[cur_tx], ARCH_DMA_MINALIGN),
			    roundup(sizeof(struct TxDesc), ARCH_DMA_MINALIGN));
	...
	do {
		rtl_inval_tx_desc(&tp->TxDescArray[cur_tx]);
		udelay(10);
...
static void rtl8168_enable_rxdvgate(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	phys_addr_t ioaddr = tp->mmio_addr;
...
static void rtl8168_disable_rxdvgate(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	phys_addr_t ioaddr = tp->mmio_addr;
...
static void rtl8168_wait_txrx_fifo_empty(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	phys_addr_t ioaddr = tp->mmio_addr;
	int i;
...
static void rtl8168_irq_mask_and_ack(phys_addr_t ioaddr)
{
	RTL_W16(IntrMask, 0x0000);
	RTL_W16(IntrStatus, RTL_R16(IntrStatus));
}

static void rtl8168_nic_reset(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	phys_addr_t ioaddr = tp->mmio_addr;
	int i;
...
static void rtl8168_hw_reset(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	phys_addr_t ioaddr = tp->mmio_addr;
...
static void rtl8168_wait_ll_share_fifo_ready(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	phys_addr_t ioaddr = tp->mmio_addr;
	int i;
...
static void rtl8168_exit_oob(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	phys_addr_t ioaddr = tp->mmio_addr;
	u16 data16;
...
static void rtl8168_rx_desc_offset0_init(struct rtl8168_private *tp, int own)
{
	...
	for (i = 0; i < NUM_RX_DESC; i++) {
		...
		flush_cache(rounddown((unsigned long)&tp->RxDescArray[i], ARCH_DMA_MINALIGN),
			    roundup(sizeof(struct RxDesc), ARCH_DMA_MINALIGN));
	}
}
...
static void rtl8168_desc_addr_fill(struct rtl8168_private *tp)
{
	...
	RTL_W32(TxDescStartAddrLow, bus_to_phys(tp->TxDescArray));
	RTL_W32(TxDescStartAddrHigh, (unsigned long)0);
	RTL_W32(RxDescAddrLow, bus_to_phys(tp->RxDescArray));
...
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	unsigned long ioaddr = tp->mmio_addr;
	u32 csi_tmp;
...
static u32 rtl8168_rx_fill(struct rtl8168_private *tp,
		struct udevice *dev, u32 start, u32 end)
{
	unsigned long rxb;
	u32 cur;
	...
	for (cur = start; end - cur > 0; cur++) {
		...
		flush_cache(rounddown((unsigned long)&tp->RxDescArray[cur], ARCH_DMA_MINALIGN),
			    roundup(sizeof(struct RxDesc), ARCH_DMA_MINALIGN));
		flush_cache(rounddown((unsigned long)tp->RxBufferRing[cur], ARCH_DMA_MINALIGN),
			    roundup(tp->rx_buf_sz, ARCH_DMA_MINALIGN));
	}

	return cur - start;
}

static int rtl8168_init_ring(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
#ifdef DEBUG_RTL8168
	int stime = currticks();
	...
	flush_cache(rounddown((unsigned long)&tp->DescArrays, ARCH_DMA_MINALIGN),
			    roundup((R8168_TX_RING_BYTES + R8168_RX_RING_BYTES + ALIGN_256), ARCH_DMA_MINALIGN));
#ifdef DEBUG_RTL8168
	printf ("%s elapsed time : %d\n", __FUNCTION__, currticks()-stime);
#endif
...
static void rtl8168_hw_d3_para(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	phys_addr_t ioaddr = tp->mmio_addr;
...
static void rtl8168_hw_init(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	phys_addr_t ioaddr = tp->mmio_addr;
...
static void rtl8168_hw_ephy_config(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	phys_addr_t ioaddr = tp->mmio_addr;
	u16 ephy_data;
...
static void rtl8168_xmii_reset_enable(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	int i, val = 0;
...
static void rtl8168_hw_phy_config(struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
...
static void rtl8168_phy_power_up (struct udevice *dev)
{
	struct rtl8168_private *tp = dev_get_priv(dev);
	phys_addr_t ioaddr = tp->mmio_addr;
	u32 csi_tmp;
...
static void rtl8168_rcalibration_setting(struct udevice *dev)
{
	...
	mdio_write(tp, 0x1f, 0xBC0);
	mdio_write(tp, 0x14, 0x14);
	mdio_write(tp, 0x17, 0xbb);
}
#endif
...
static void rtl8168_get_mac_address(struct udevice *dev)
{
	...
	char new_ethaddr[20];
#endif
#ifdef DEBUG_RTL8168_INIT
	printf("enter gen mac\n");
#endif
#ifdef CONFIG_ENV_IN_RAM
	strcpy(plat->enetaddr, MK_STR(CONFIG_ETHADDR));
	/*new_ethaddr[12] = 0;
	sprintf(new_ethaddr, "%s%02x:%02x", new_ethaddr, random(), random());*/
	printf("new_ethaddr = %s\n", plat->enetaddr);

	eth_env_set_enetaddr_by_index("eth", dev_seq(dev), plat->enetaddr);
...
#define REG_BASE    0x18400000
#ifdef CONFIG_FT2_TEST_ETHERNET
void fephy_write(u32 page, u32 reg, u32 data)
{
	u32 temp, rdata;
	...
	TEST_MEM32(0x18400060) = temp;
	while ((rdata & 0x80000000) != 0x0)
		rdata = TEST_MEM32(0x18400060);
	/*write reg*/
	rdata = 0xffffffff;
...
u32 fephy_read(u32 page, u32 reg)
{
...
#endif
#ifdef _ETH_LOOPBACK_TEST_
static void eri_write(u32 eri_wdata, u32 eri_addr, u32 eri_byteen)
{
	u32 temp, rdata;
	rdata = 0xffffffff;
	temp = 0x80000000 + ((eri_byteen & 0xf)<<12) + eri_addr;
	TEST_MEM32(0x18400070) = eri_wdata;/*ERIDR=0x18400070*/
	TEST_MEM32(0x18400074) =  temp;/*ERIAR=0x18400074*/
	while ((rdata & 0x80000000) != 0x0) {
		rdata = TEST_MEM32(0x18400074);
	}
}


static u32 eri_read(u32 eri_addr)
{
	u32 wdata, rdata;
	wdata = eri_addr & 0xfff;
	TEST_MEM32(0x18400074) = wdata;/*ERIAR=0x18400074*/
	rdata = TEST_MEM32(0x18400074);
	while ((rdata & 0x80000000) == 0x0) {
		rdata = TEST_MEM32(0x18400074);
	}
	rdata = TEST_MEM32(0x18400070);/*ERIDR=0x18400070*/
	return rdata;
}

static void basic_cfg(u32 tx_dsc_st_addr, u32 rx_dsc_st_addr)
{
	u32 rdata, wdata;
	rdata = 0x0;
	printf("basic cfg!\n");

	/*rdata = READ_REG_32(ETHERNET_CLK_CFG);//ETHERNET_CLK_CFG=0x18861024
	wdata = rdata | 0x00020000; //bit rdata[16]: clk250m_enable
	WRITE_REG_32(ETHERNET_CLK_CFG, wdata);*/

	rdata = TEST_MEM32(0x18400054);/*CFG345_TDFNR=0x18400054*/
	wdata = (rdata & 0xff000000)+0x020026;
	TEST_MEM32(0x18400054) = wdata;

	TEST_MEM32(0x1840000c) = 0xffffffff;/*MAR03=0x1840000C*/
	TEST_MEM32(0x18400008) = 0xffffffff;/*MAR47=0x18400008*/
	TEST_MEM32(0x18400000) = 0x004c5452;/*MACID0_01=0x18400000*/

	rdata = TEST_MEM32(0x18400004);/*MACID0_2=0x18400004*/
	wdata = (rdata & 0xffff0000)+0x0800;
	TEST_MEM32(0x18400004) =  wdata;

	TEST_MEM32(0x18400020) = (tx_dsc_st_addr-0xA0000000);/*TXQ0_DSC_ST_ADDR_01=0x18400020*/

	rdata = TEST_MEM32(0x18400054);/*CFG345_TDFNR=0x18400054*/
	wdata = (rdata & 0x00ffffff)+0x04000000;
	TEST_MEM32(0x18400054) = wdata;

	TEST_MEM32(0x184000E4) = (rx_dsc_st_addr-0xA0000000);/*RDSAR_01=0x184000E4*/

	TEST_MEM32(0x1840003c) =  0xffff0000;/*IMR0=0x1840003C*/

	rdata = TEST_MEM32(0x184000e0);/*CCR=0x184000E0*/
	wdata = (rdata & 0xffff0000)+0x0c61;
	TEST_MEM32(0x184000e0) = wdata;

	rdata = TEST_MEM32(0x184000d8);/*RMS=0x184000D8*/
	wdata = (rdata & 0x0000ffff) + 0x30000000;
	TEST_MEM32(0x184000d8) = wdata;

	rdata = TEST_MEM32(0x184000ec);/*MTPS=0x184000EC*/
	wdata = (rdata & 0xffffff00)+0x0f;
	TEST_MEM32(0x184000ec) = wdata;

	eri_write(0x00080002, 0xc8, 0xf);/*call function eri_write()*/
	eri_write(0x00000038, 0xcc, 0xf);
	eri_write(0x00000048, 0xd0, 0xf);
	eri_write(0x00100006, 0xe8, 0xf);
	rdata = eri_read(0xdc);
	eri_write((rdata & 0xffff0000)+0xd, 0xdc, 0xf);

	TEST_MEM32(0x18400040) =  0x03000380;/*TCR_01=0x18400040*/

	rdata = 0x0;
	while ((rdata & 0x02000000) == 0x0) {
		rdata = TEST_MEM32(0x184000d0);/*PHY_PWR_FUNC=0x184000D0*/
	}

	TEST_MEM32(0x18400044) = 0x0000080e;/*CR_01=0x18400044*/

	rdata = TEST_MEM32(0x18400034);/*CR=0x18400034*/
	wdata = (rdata & 0x00ffffff)+0x8c000000;/*tx/rx enble*/
	TEST_MEM32(0x18400034) = wdata;

	rdata = TEST_MEM32(0x18400034);/*CR=0x18400034*/
	wdata = (rdata & 0x00ffffff)+0x0c000000;/*disable stop all request*/
	TEST_MEM32(0x18400034) = wdata;

	TEST_MEM32(0x18400044) = 0x0000080e;/*RCR_01=0x18400044*/

	TEST_MEM32(0x18400010) = 0x0f000000;/*DTCCR_01=0x18400010*/
}



static int check_pcs_loopback_data(u32 tx_data_addr[], u32 tx_packet_seg[], u32 tx_data_length[], u32 rx_buf_st_addr[], u32 dsc_num_tx, u32 pkt_num)
{
	u8 tx_data_temp[1518];
	u8 rx_data_temp[1518];
	u32 rx_data_length;
	int index;
	u32 temp;
	int i, j, k, m;
	...
	printf("check_pcs_loopback_data!\n");

	for (i = 0; i < pkt_num; i++) {
		for (j = k; j < dsc_num_tx; j++) {
			for (m = 0; m < tx_data_length[j]/4+2; m++) {
				switch (tx_data_addr[j]%4) {
				case 0:{
					temp = TEST_MEM32(tx_data_addr[j]+m*4);
					tx_data_temp[index] = temp;
					index++;
					tx_data_temp[index] = temp>>8;
					index++;
					tx_data_temp[index] = temp>>16;
					index++;
					tx_data_temp[index] = temp>>24;
					index++;
					break;
				       }
				case 1:	{
			...
			rx_data_length = rx_data_length + tx_data_length[j];
			index = tx_data_length[j];
			if ((tx_packet_seg[j]&0x1) == 0x1) {
				k = j+1;
				index = 0;
				break;
			}
		...
		for (m = 0; m < rx_data_length/4+2; m++) {
			switch (rx_buf_st_addr[i]%4) {
			case 0:	{
				temp = TEST_MEM32(rx_buf_st_addr[i]+m*4);
				rx_data_temp[index] = temp;
				index++;
				rx_data_temp[index] = temp>>8;
				index++;
				rx_data_temp[index] = temp>>16;
				index++;
				rx_data_temp[index] = temp>>24;
				index++;
				break;
				}
			case 1:	{
		...
		for (m = 0; m < rx_data_length; m++) {
			if (tx_data_temp[m] != (rx_data_temp[m])) {
				printf("data mismatch! ERR!%x, %x, %x\n", m, tx_data_temp[m], rx_data_temp[m]);
				return -1;
			}
		}
		printf("this packet compares successfully!\n");
...
static void start_pcs_loopback(u32 mode)
{ /*mode: 0, 10M; 1, 100M*/

	/*u32 wdata, rdata;*/
	/*1: power on reset*/
	/*2: speed up mode, page0 reg28 0x00c7*/
	fephy_write(0, 28, 0x00c7);
	/*3: interrupt enable, page7 reg19 0xf834*/
	fephy_write(7, 19, 0xf834);
	if (mode == 0) {
		/*4: set analbk10m=1, page0 reg24 0x0c00*/
		fephy_write(0, 24, 0x0c00);
		/*5: set loopback=1, set duplex mode=1, page0 reg0 0x4100*/
		fephy_write(0, 0, 0x4100);
	} else if (mode == 1) {
	/*4	: set mode*/
		/*rdata = fephy_read(0, 26);
		wdata = rdata | 0x0C00;
		fephy_write(0, 26, wdata);
		5: set testfun=1, page0 reg28 0x00c7
		fephy_write(0, 28, 0x00c7);
		fephy_write(0, 28, 0x00e7);
		6: forcelink, page0 reg0 0x6100*/
		fephy_write(0, 0, 0x6100);
	}
}
...
static void start_TX(void)
{
	...
	rdata = TEST_MEM32(0x18400038);/*TPPOLL=0x18400038*/
	wdata = (rdata & 0xffffff00) + 0x40;
	TEST_MEM32(0x18400038) = wdata;
}

static u32 rand32(u32 seed_tmp)
{
	u32 seed_value;
	seed_value = (seed_tmp << 1) ^ ((seed_tmp & 0x80000000) ? POLY16 : 0);
	return seed_value;
}


static void set_TX_dsc(u32 tx_dsc_st_addr, u32 tx_data_addr[], u32 tx_data_length[], u32 tx_packet_seg[], u32 dsc_num)
{
	int i;
	u32 dsc_st_addr;
	printf("set_TX_dsc!\n");
	for (i = 0; i < dsc_num; i++) {
		dsc_st_addr = tx_dsc_st_addr+i*16;
		if (i == dsc_num-1) {
			TEST_MEM32(dsc_st_addr) =  0xc0000000+(tx_packet_seg[i]<<28) + (tx_data_length[i]&0xffff);
		} else {
			TEST_MEM32(dsc_st_addr) = 0x80000000+(tx_packet_seg[i]<<28) + (tx_data_length[i]&0xffff);
		}
		TEST_MEM32(dsc_st_addr+4) =  0x00000000;
		TEST_MEM32(dsc_st_addr+8) =  tx_data_addr[i]-0xA0000000;
		TEST_MEM32(dsc_st_addr+12) = 0x00000000;
	}
}


static void set_RX_dsc(u32 rx_dsc_st_addr, u32 rx_buf_st_addr[], u32 rx_buf_length[], u32 dsc_num)
{
	int i;
	u32 dsc_st_addr;
	...
	for (i = 0; i < dsc_num; i++) {
		dsc_st_addr = rx_dsc_st_addr+i*16;
		if (i == dsc_num-1) {
			TEST_MEM32(dsc_st_addr) = 0xc0000000+(rx_buf_length[i]&0x3fff);
		} else {
			TEST_MEM32(dsc_st_addr) = 0x80000000+(rx_buf_length[i]&0x3fff);
		}
		TEST_MEM32(dsc_st_addr+4) =  0x00000000;
		TEST_MEM32(dsc_st_addr+8) =  rx_buf_st_addr[i]-0xA0000000;
		TEST_MEM32(dsc_st_addr+12) = 0x00000000;
	}
}


static int eth_loopback(void)
{

	u32 tx_dsc_st_addr;
	u32 rx_dsc_st_addr;
	u32 tx_data_st_addr;
	/*u32 tx_data_init_addr;
	u32 tx_data_init_length;
	u32 rx_data_init_addr;
	u32 rx_data_init_length;*/
	u32 dsc_num_tx, dsc_num_rx, pkt_num;
	u32 tx_data_addr[64];
	u32 tx_data_length[64];
	/*u32 rx_mem_data_addr[64];
	u32 rx_data_length[64];*/
	u32 tx_packet_seg[64];
	u32 rx_buf_st_addr[64];
	u32 rx_buf_length[64];
	u32 seed;
	int i;
	int ret;

/*
	unsigned char efuse_data[24] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
					0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
					0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
*/
	tx_dsc_st_addr = 0xA0600000;
	rx_dsc_st_addr = 0xA0600400;
	...
	**********************************************/
	...
	tx_data_st_addr = 0xA0600800;
	/*
	tx_data_init_length = 0x2600;

	tx_data_init_addr = 0x1d018000;
	*/
	tx_data_addr[0]  = 0xA0600800;
	tx_data_addr[1]  = 0xA0600E00;
	tx_data_addr[2]  = 0xA0601400;

	tx_data_length[0] = 0x5DC;/*1500byte */
	tx_data_length[1] = 0x401;/*1025byte*/
	tx_data_length[2] = 0x40;/*64byte*/

	tx_packet_seg[0] = 0x3;/*FS:1, LS:1*/
	tx_packet_seg[1] = 0x2;/*FS:1, LS:0*/
	tx_packet_seg[2] = 0x1;/*FS:0, LS:1*/

	rx_buf_st_addr[0]   = 0xA0604000;
	rx_buf_st_addr[1]   = 0xA0604600;
	rx_buf_st_addr[2]   = 0xA0604C00;

	rx_buf_length[0]  = 0x5EE;/*1518 Byte*/
	rx_buf_length[1]  = 0x5EE;
	rx_buf_length[2]  = 0x5EE;

	/*  force_ethernet_reset();*/

	/*reset_fephy();*/

	/*udelay(2000);*/

	/*release_ethernet_reset();*/

	seed = 0x12345678;
	for (i = 0; i < seed; i = i+4) {
	seed = rand32(seed) ;
	TEST_MEM32(tx_data_st_addr + i) = seed;
	}

	/*Set Mac ID*/
	for (i = 0; i < dsc_num_tx; i++) {
	TEST_MEM32(tx_data_addr[i]) =   0xFFFFFFFF;
	TEST_MEM32(tx_data_addr[i]+4) = 0xFFFFFFFF;
	TEST_MEM32(tx_data_addr[i]+8) = 0xFFFFFFFF;
	}

	set_TX_dsc(tx_dsc_st_addr, tx_data_addr, tx_data_length, tx_packet_seg, dsc_num_tx);

	set_RX_dsc(rx_dsc_st_addr, rx_buf_st_addr, rx_buf_length, dsc_num_rx);

	mdelay(3);

	basic_cfg(tx_dsc_st_addr, rx_dsc_st_addr);

	start_pcs_loopback(1);/*100M*/
	...
	ret = check_pcs_loopback_data(tx_data_addr, tx_packet_seg, tx_data_length, rx_buf_st_addr, dsc_num_tx, pkt_num);  /*takes too much time*/

	mdelay(1);

	printf("ETHERNET Simulation OK!\n");
	return ret ;
}
#endif


static u16 read_otp_group0_fephy(void)
{
	u32 addr_t[4];

	rts_otp_read((u32)addr_t, 0, 16);
	return (addr_t[2] >> 16) & 0xff;
}

int rtl8168_initialize(struct udevice *dev, const char *name)
{
//	pci_dev_t devno = 0;
	int card_number = 0;
//	struct eth_device *dev;
	...
	/*reset FEPHY, hold */
/*
 *	tmp2 = rtd_inl(FORCE_REG_RST_FWC);
 ...
 */
	...
	/*select external phy*/
	rtd_outl(ETHERNET_LDPHY_CFG, LDPHY_SEL | RMII_REF_CLK_EN | SPD);
	rtd_outl(MIPI_TX_PAD_CFG, ETN_RMII_CFG);

	/*set gpio7 to input, for interrupt*/
	gpio_direction_input(7);
	rtd_outl(ETHERNET_LDPHY_CFG2, INT_GPIO7);

	/*clock select*/
	rtd_outl(MAC_BYPASS_CLK_CFG_REG, MAC_BYPASS_CLK_EN |
			MIPI_BYPASS_CLK_GPLL0_DIV2);
	...
	/*set fephy address*/
	tmp2 = rtd_inl(ETHERNET_CFG_REG);
	rtd_outl(ETHERNET_CFG_REG, tmp2 | ETH_EPHY_ADDR | ETH_EPHY_CLK_BYPASS);
#else
	...
	/*set ldo to 1.0v*/
	rtd_outl(LDO_FEPHY_CFG, 0x49);

	/*open ldo*/
	rtd_outl(FEPHY_LDO_CTRL, 0x3);

	/*FEPHY_ISO_EN set to 0*/
	rtd_outl(ETH_PDWN_CONTROL, 0);

	/*set fephy address*/
	tmp2 = rtd_inl(ETHERNET_CFG_REG);
	rtd_outl(ETHERNET_CFG_REG, tmp2 | ETH_EPHY_ADDR);
#endif

	/* set fephy from otp*/
	u16 otp_value = read_otp_group0_fephy();
	// printf("~~~~~~~~~~~~~~otp_value %d~~~~~~~~~~~~~~~~~\n", otp_value);

	if (otp_value & 0x2000) {
		rdata = fephy_read(0xbc0, 0x14);
		fephy_write(0xbc0, 0x14, rdata | ((otp_value >> 1) & 0xf) |
			    ((otp_value << 4) & 0x10));
		rdata = fephy_read(0xbc0, 0x17);
		fephy_write(0xbc0, 0x14, rdata | ((otp_value >> 5) & 0xff));
	}

#ifdef CONFIG_TARGET_FPGA
	tmp1 = rtd_inl(ETHERNET_LDPHY_CFG2) & 0xFFFF;
	rtd_outl(ETHERNET_LDPHY_CFG2, tmp1 | RMII_RX_CLK_INV | TX_PHASE_SEL);
#endif

	/*enable MAC clock*/
	tmp2 = rtd_inl(ETHERNET_CLK_CFG_REG);
	rtd_outl(ETHERNET_CLK_CFG_REG, tmp2 | CLK_ENABLE);
	mdelay(20);
...
#else
	tmp1 = rtd_inl(0x18860008);

	/*reset MAC, hold */
	rtd_outl(0x18860008, tmp1|0x40);

	/*reset FEPHY, hold */
	tmp2 = rtd_inl(0x18861024);
	rtd_outl(0x18861024, (tmp2&0xffffbfff));
	mdelay(20);

	/*release FEPHY, and enable MAC clock*/
	tmp2 = rtd_inl(0x18861024);
	rtd_outl(0x18861024, tmp2|0x20000|0x4000|0x200);
	mdelay(20);

	/*release MAC*/
	rtd_outl(0x18860008, tmp1&0xFFFFFFBF);

#endif


	/*reset fephy, set page0, reg0, bit15*/
	rdata = fephy_read(0, 0);
	fephy_write(0, 0, rdata|0x8000);
	mdelay(20);

#if defined(CONFIG_ETH_EFUSE_INIT)
	volatile u32 EFUSEDAT[2] = {SYS_EFUSE_OUT0, SYS_EFUSE_OUT1};
	for (i = 0; i < 8; i++) {
		u8 efuseread;
		u32 efusedata;
		if (i % 4 == 0)
			efusedata = 0;
		efuseread = efuse_read_byte(i);
		efusedata |= (efuseread << ((3 - i % 4) * 8));
		if ((i + 1) % 4 == 0)
			EFUSEDAT[i / 4] = efusedata;
	}
#endif
	/*efuse ready*/
//	rtd_outl(SYS_EFUSE_CTL, EFUSE_DONE);

#ifdef DEBUG_RTL8168_INIT
	printf("set eth clk %x\n", rtd_inl(ETHERNET_CLK_CFG_REG));
#endif
	/* Find RTL8168 */
...
//	}
	...
	tp->mmio_addr = iobase;
#ifdef DEBUG_RTL8168_INIT
	printf("af pci_mem_to_phys\n");
#endif
//	tp->devno = devno;
//	tp->dev = dev;
	tp->rx_buf_sz = RX_BUF_SIZE;

#ifndef _ETH_OOB_TEST_
	/* Identify chip attached to board */
	rtl8168_get_mac_version(tp, tp->mmio_addr);
#ifdef DEBUG_RTL8168_INIT
	printf("af rtl8168_get_mac_version\n");
#endif
...
//	dev->write_hwaddr = rtl8168_write_hwaddr;
//	eth_register (dev);
#ifdef _ETH_PATCH_
		/*snr patch*/
		fephy_write(1, 0x1d, 0x000f);
		fephy_write(1, 0x10, 0x7181);

		/*100M TX slew rate control, bit4:3, from 0x3 to 0x1*/
		fephy_write(1, 0x11, 0x77ac);

		/*set xmii speed*/
		fephy_write(0, MII_ADVERTISE, ADVERTISE_100FULL|ADVERTISE_100HALF|ADVERTISE_10FULL|ADVERTISE_100HALF);
		fephy_write(0, MII_BMCR, BMCR_ANENABLE | BMCR_ANRESTART | BMCR_RESET);
#endif

	/*interrupt enable, page7 reg19 0xf834*/
	fephy_write(7, 19, 0xf834);
	card_number++;
	...
	i = 0;
	while (tmp1 == 0) {
		printf(" eth self loopback test, cycle :0x%x\n", i);
		tmp1 = eth_loopback();
		i++;
		}
#endif
	/* LED setting*/
	/*LED1 for 100M link , LED2 for 10M link*/
	rtd_outl((REG_BASE+CustomLED), 0x00060ca9);
...
//{
//	struct eth_pdata *plat = dev_get_plat(dev);
//	struct rtl8168_private *priv = dev_get_priv(dev);

//	rtl8168_common_start(dev, plat->enetaddr, priv->iobase);

//	return 0;
//}
//#endif
#ifdef CONFIG_DM_ETH
static int rtl8168_write_hwaddr(struct udevice *dev)
{
	struct eth_pdata *plat = dev_get_plat(dev);
	unsigned long ioaddr = plat->iobase;
	unsigned int i;
...
#ifdef CONFIG_DM_ETH
static int rtl8168_eth_probe(struct udevice *dev)
{
...
//	struct rtl8168_private *priv = dev_get_priv(dev);
	...
	iobase = 0x18400000;
	iobase &= ~0xf;
	plat->iobase = (int)dm_pci_mem_to_phys(dev, iobase);

	debug("rtl8168: REALTEK RTL8168 @0x%lx\n", iobase);
	switch (pplat->device) {
	case 0x8168:
		region = 2;
		break;
...
//	dm_pci_read_config32(dev, PCI_BASE_ADDRESS_0 + region * 4, &iobase);
	...
	ret = rtl8168_initialize(dev, dev->name);
	if (ret < 0) {
...
static const struct eth_ops rtl8168_eth_ops = {
	.start	= rtl8168_init,
	.send	= rtl8168_send,
	.recv	= rtl8168_recv,
	.stop	= rtl8168_halt,
...
//U_BOOT_PCI_DEVICE(eth_rtl8168, supported);
#endif
